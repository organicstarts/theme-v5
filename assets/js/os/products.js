...
const renderProducts = () => {
    ...
    if(arrivalElement || nextBusinessDayElement) {
        if (timeleft >= 0 && timeleft <= 24 && currentMoment.isBusinessDay()) {
            setInterval(() => {
                today = new Date();
                nextBusinessDayElement && (nextBusinessDayElement.innerHTML = `${(cutoff - today) > 0 ? `today if ordered within ${dhm(cutoff - today)}.` : `on ${weekday[next.getDay()]}, ${month[next.getMonth()]} ${next.getUTCDate()}.`}`);
                arrivalElement && (arrivalElement.innerHTML = (cutoff - today) > 0 ? arrival : '');
            }, 1000);
        } else {
            if (Math.floor((next - today) / (24 * 60 * 60 * 1000)) < 2) {
                nextBusinessDayElement && (nextBusinessDayElement.innerHTML = `on ${weekday[next.getDay()]}, ${month[next.getMonth()]} ${next.getUTCDate()}.`);
                arrivalElement && (arrivalElement.innerHTML = arrival);
            } else {
                nextBusinessDayElement && (nextBusinessDayElement.innerHTML = 'securely from our Nevada fulfillment center.');
            }
        }
    }
